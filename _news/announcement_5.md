---
layout: post
date: 2025-11-14 19:00:00-0400
inline: false
---

Exciting Opportunity: PhD/Postdoctoral Positions Available (Updated!)

I am pleased to announce that PhD and Postdoctoral positions are open under my supervision in the following areas:

 1. Information-Geometry Based Filtering and Parameter Estimation
 2. Data-Driven Nonlinear Control Design and Analysis

For postdoctoral candidates, we are seeking individuals with a maximum of four years of experience post-PhD. Recent PhD graduates or soon-to-be graduates are encouraged to apply. Candidates should have strong skills in mathematics, statistics, and scientific programming. Backgrounds in control theory, statistics, computer science, applied mathematics, or related fields are ideal.

The postdoctoral position is initially for one year, with potential extensions based on funding and performance. Compensation is aligned with the university's postdoctoral salary structure.

Application Requirements:

 * Updated CV (including education history and work experience)
 * List of publications
 * Three most impactful research papers
 * Research statement
 * References
 * Copies of diplomas and university transcripts

Interested applicants are kindly requested to complete this form: https://lnkd.in/d7TFtd4u

The positions will remain open until filled with the right candidates. We appreciate all applications; however, only shortlisted candidates will be contacted.

[IRCSmartMobilityAndLogistics](https://lnkd.in/d4vGSRwr)